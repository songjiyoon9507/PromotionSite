...
checkAuthKeyBtn.addEventListener("click", () => {
    ...
    if(min === 0 && sec === 0) { // 타이머가 00분 00초인 경우
    ...
    // 입력받은 이메일, 인증번호로 객체 생성해서 body에 보낼 거
    const obj = {
    ...
    fetch("/email/checkAuthKey", {
        method : "POST",
        headers : {"Content-Type" : "application/json"},
        body : JSON.stringify(obj) // obj를 JSON 형태로 변환해서 보냄
    })
    .then(resp => resp.text())
    .then(result => {
...
memberNickname.addEventListener("input", e => {
    ...
    /* test 안에 inputNickname 안 넣어서 유효성 검사 제대로 안 된 거 */

    // 2) 정규식 검사
    const regExp = /^[가-힣\w\d]{2,10}$/;

    if(!regExp.test(inputNickname)) { // 정규식 검사 유효하지 않을 때
        nickMessage.innerText = "닉네임이 유효하지 않습니다.";
        nickMessage.classList.add("error");
        nickMessage.classList.remove("confirm");
        checkObj.memberNickname = false;
    ...
    - GET 요청
    - 응답 결과 1 / 0 과 같은 Count 로 할 것
    - 1 : 중복 O -> 이미 사용중인 닉네임입니다. (빨간 글씨)
    - 0 : 중복 X -> 사용 가능한 닉네임입니다. (초록 글씨)
     */
    fetch("/member/checkNickname?memberNickname=" + inputNickname)
    .then(resp => resp.text())
    .then(count => {
...
signUpForm.addEventListener("submit", e => {
    ...
    // for ~ in 문 (객체 전용 향상된 for 문)
    for(let key in checkObj) { // checkObj 객체 요소의 key 값을 순서대로 꺼내옴
